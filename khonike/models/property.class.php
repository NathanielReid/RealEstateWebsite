...
class Property extends Dbh {
    ...
    // Property Search
    public function getPropertyBySearch($city, $propertyType, $propertyStructure, $propertyPrice) {
        $sql = "SELECT * FROM property WHERE city LIKE '$city' AND property_type LIKE '$propertyType' AND structure LIKE '$propertyStructure' AND price BETWEEN '100' AND '$propertyPrice'";
        $stmt = $this->connect()->prepare($sql);
        $stmt->execute([$city, $propertyType, $propertyStructure, $propertyPrice]);
